...
public class Syncee {
    ...
    private void requestFilterID(DataCallback filterIDResponse) {
        JSONObject object = new JSONObject(
            "{ \"room\": { \"state\": { \"types\": [ \"m.room.*\" ] } }, \"presence\": { \"types\": [ \"m.presence\" ] }, \"event_fields\": [ \"type\", \"content\", \"sender\", \"room_id\", \"event_id\" ] }");
        try {
            httpHelper.sendRequestAsync(
                c.getHost(),
                HttpHelper.URLs.user + c.getLoginData()
                    .getUser_id() + "/filter",
                object,
                data -> {
                    try {
    ...
    private void runEventListener(String filterID) {
        if (eventListenerThread == null) {
            eventListenerThread = new Thread(() -> {
                String baseurl = HttpHelper.URLs.sync + "?access_token="
                    + c.getLoginData()
                        .getAccess_token()
                    + "&filter="
                    + filterID
