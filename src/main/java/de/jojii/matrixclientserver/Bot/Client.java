...
public class Client {
     ...
     */
    public void loginWithJWTSync(String jwtToken, String deviceId) throws IOException {
        JSONObject object = new JSONObject();
    ...
    public void setPresence(String userid, String presence, String msg, EmptyCallback onStateChanged)
        throws IOException {
        if (!isLoggedIn()) return;
    ...
    public String getOrCreateDirectChatRoomSync(String userID) throws IOException {
        ...
        String response = httpHelper
            .sendRequest(host, HttpHelper.URLs.client + "createRoom", jsonObject, true, "POST", true);
        ...
        httpHelper.sendRequest(
            host,
            HttpHelper.URLs.user + loginData.getUser_id() + "/account_data/m.direct",
            mDirect,
            true,
            "PUT");
    ...
    public Map<String, List<String>> getDirectChatRoomsMapSync() throws IOException {
        ...
        String response = httpHelper.sendRequest(
            host,
            HttpHelper.URLs.user + loginData.getUser_id() + "/account_data/m.direct",
            null,
            true,
                ...
                Collectors.toMap(
                    ...
                    key -> IntStream.range(
                        ...
                        jsonObject.getJSONArray((String) key)
                            .length())
                        .mapToObj(
                            i -> jsonObject.getJSONArray((String) key)
                                .getString(i))
    ...
    public String resolveRoomAliasSync(String roomID) throws IOException {
        if (roomID.startsWith("#")) {
            String response = httpHelper
                .sendRequest(host, HttpHelper.URLs.directory + "room/" + roomID, null, true, "GET");
            JSONObject object = new JSONObject(response);
    ...
    public void sendText(String roomID, String message, boolean formatted, String formattedMessage,
        DataCallback response) throws IOException {
        if (!isLoggedIn()) return;
    ...
    public void sendRoomEvent(String event, String roomID, JSONObject content, DataCallback response)
        throws IOException {
        httpHelper.sendRequestAsync(
            host,
            HttpHelper.URLs.rooms + roomID + "/send/" + event + "/" + System.currentTimeMillis(),
            content,
            "PUT",
            data -> {
                if (response != null) {
    ...
    public void sendReadReceipt(String roomID, String eventID, String receiptType, DataCallback response)
        throws IOException {
        if (!isLoggedIn()) return;

        httpHelper.sendRequestAsync(
            host,
            HttpHelper.URLs.rooms + roomID + "/receipt/" + receiptType + "/" + eventID,
            null,
            "POST",
            data -> {
                if (response != null) {
    ...
    public void setTyping(boolean typing, String userid, String roomID, int timeout, EmptyCallback response)
        throws IOException {
        if (!isLoggedIn()) return;
            ...
            if (memberCallback != null) {
                try {
                    ...
                    while (keys.hasNext()) {
                        ...
                        String avatar = "";
                        if (user.has("avatar_url") && user.get("avatar_url") != null
                            && user.get("avatar_url") instanceof String) {
                            avatar = user.getString("avatar_url");
                        }
                        members.add(new Member(key, user.getString("display_name"), avatar));
                    }
    ...
    public void getRoomEventFromId(String roomID, String eventID, RoomEventCallback callback) throws IOException {
        ...
        httpHelper.sendRequestAsync(
            host,
            HttpHelper.URLs.rooms + URLEncoder.encode(roomID) + "/event/" + URLEncoder.encode(eventID),
            null,
            "GET",
            data -> {
                if (callback != null) {
    ...
    public void createRoom(String preset, String visibility, String alias, String name, String topic,
        List<String> invitations, String roomVersion, DataCallback callback) throws IOException {
        if (!isLoggedIn()) return;
    ...
    public void sendFile(String contentType, int contentLength, InputStream data, DataCallback callback)
        throws IOException {
        httpHelper.sendStreamAsync(
            host,
            HttpHelper.URLs.upload,
            contentType,
            contentLength,
            data,
            true,
    ...
    public static class Room {
        ...
        public static String public_chat = "public_chat", private_chat = "private_chat",
    ...
    public Client(String host) {
        this.host = host;
        this.httpHelper = new HttpHelper(() -> { return loginData != null ? loginData.getAccess_token() : null; });
